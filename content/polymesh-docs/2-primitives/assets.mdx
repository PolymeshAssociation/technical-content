...
External agents are other Polymesh identities that have been granted permissions related to a particular agent. Their role is to manage the asset on behalf of the asset issuer.

External agents can be added to an asset by the asset issuer by issuing a `BecomeAgent` authorisation. The external agents identity then needs to accept this authorisation.
