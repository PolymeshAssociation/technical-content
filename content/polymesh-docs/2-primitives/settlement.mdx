...
If an instruction is fully affirmed, but settlement fails (for example as a result of compliance rules) the instruction can be re-settled again at a later date by any one of the counterparties, once the failure cause has been remediated.
...
Payment receipts are small pieces of data signed by an authorised signatory (determined by the venue on which the instruction is settling) which allege that the corresponding assets have moved off-chain.
