...
- `InvestorZKProof`: These are self-issued claims (an identity makes the claim about itself) that declare any [linkage](./confidential-identity) between identities owned by the same entity investing in the same asset.
