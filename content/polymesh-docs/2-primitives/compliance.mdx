...
- `InvestorZKProof`: These are self-issued claims (an identity makes the claim about itself) that declare any [linkage](./confidential_identity) between identities owned by the same entity investing in the same asset.
...
In addition to the flexible claim based compliance manager, asset issuers can use transfer restriction [smart extensions](./smart_extensions) to further control the trading of their assets.