...
Currently we take a linear count of positive and negative signals and use these to order PIPs, but in the future we can consider more expressive mechanisms, such as quadratic voting by leveraging our [confidential identity](./confidential_identity) model.
...
The Goverance Council is also able to issue a [Customer Due Diligence](./cdd) claim should the need arise.
