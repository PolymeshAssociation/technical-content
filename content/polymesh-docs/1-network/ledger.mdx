...
NB - due to memory constraints, the version of the app for the Ledger Nano S only supports a subset of possible transactions, primarily POLYX transfers, identity and staking operations.
...
- The latest firmware installed (at the time of writing this is 2.0.0 on the Nano S, and 1.3.0 on the Nano X).
...
- Find Polymesh in the app catalog and install it. You will need to enable Developer Mode in LedgerLive to see the app in the catalog.
...
Now when you go to the *"Accounts"* tab you will see a new button that says *"Add Ledger"*. Ensure that your Ledger device is unlocked and that you have navigated into the Polymesh application, then click this button. Whilst the app is in review, you will also need to click through when it tells you that the app is "Pending Ledger Review".
