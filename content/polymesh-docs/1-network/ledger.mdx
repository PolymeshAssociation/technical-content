...
NB - due to memory constraints, the version of the app for the Ledger Nano S only supports a subset of possible transactions, primarily POLYX transfers and staking operations.
...
- Ledger Live is installed and at version 2.1 or newer (see *settings -> about* to find out if you're
  up to date).
- A Chromium-based web browser installed to access the [Polymesh App](https://itn-app.polymesh.live/#/explorer) with.
...
The [Polymesh App](https://itn-app.polymesh.live/#/accounts) already has an integration with the Ledger application so that your device will work with the browser interface after installation. 
