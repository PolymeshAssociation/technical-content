...
description:
slug: /polymesh-docs/network/running_a_node_docker
...
```
sudo docker pull polymeshassociation/polymesh:4.1.2-testnet-distroless
```

The tag `:4.1.2-testnet-distroless` should be replaced with the desired release tag. 
...
```
sudo docker run --rm polymeshassociation/polymesh:4.1.2-testnet-distroless --version
```

**All available Polymesh [FLAGS], [OPTIONS] and <SUBCOMMAND\> commands can be viewed by calling:**

```
sudo docker run --rm polymeshassociation/polymesh:4.1.2-testnet-distroless --help
```
...
```
sudo docker run -u root --rm -it polymeshassociation/polymesh:4.1.2-testnet-distroless --chain testnet
```
...
polymeshassociation/polymesh:4.1.2-testnet-distroless \
--chain testnet \
