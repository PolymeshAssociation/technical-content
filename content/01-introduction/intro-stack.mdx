...
Polymesh's rules are defined as "extrinsics", which are extensions to the blockchain protocol itself. In other words, there is a similarity to smart contracts but extrinsics are implemented at a lower level.

Developers can extend Polymesh by creating Substrate extensions. For example, extend an asset with a custom logic restricting transfers in a way that is not anticipated by the existing network.
...
The **Polymesh Dashboard** is a multi-faceted Web GUI that is in many ways like a blockchain explorer. Again, we see the advantage of a purpose-built chain as compared to a generalised chain.
...
- SDK: https://developers.polymath.network/polymesh-sdk-api-reference/
