...
We will take a closer look at Polymesh's governance system at a later point of time. In case you want to dive right into it, just go to the section on [governance](/introduction/intro-governance)
...
Borrowing from [Substrate](https://substrate.dev/docs/en/knowledgebase/learn-substrate/extrinsics), the term **extrinsics** describes _any outside information that enters the blockchain_, typically user-signed transactions submitted to the blockchain. So you may come across wording like the "create token extrinsic", which is none other than a properly formatted transaction that expresses the intent to create a token.
...
You want to know more about the Polymesh Wallet? Take a look at our section on the [Wallet](/introduction/quickstart-wallet)!
...
For a step-by-step on how to setup everything, so that you can start playing around with the Dashboard, take a look at [Quick Start](/quick-start/quickstart-landingpage).
