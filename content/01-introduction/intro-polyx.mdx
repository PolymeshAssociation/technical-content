...
* POLY on Ethereum's Kovan Testnet, which one gets through the POLY bridge to convert to POLYX on Polymesh's Testnet. It is expected that this network, its future test siblings, and its token remain a playground and play token used for testing only;
