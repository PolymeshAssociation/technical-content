...
There are two general methods of acquiring POLYX. 
1. A third party can send POLYX to your address.
2. One can upgrade their POLY tokens on the Ethereum blockchain. 
In either case, you will need to complete the one-time [customer due diligence (CDD)](/quickstart/verification-with-cdd) process on your Polymesh account before such actions are possible. 
Let us address the upgrade process. We will use it to [acquire testnet POLYX](/quickstart/quickstart-polyx) later.
...
* POLY on Ethereum's Kovan Testnet, which one gets through the POLY bridge to upgrade to POLYX on Polymesh's Alcyone testnet. It is expected that this network, its future test siblings, and its token remain a playground and play token used for testing only;
* POLY on Ethereum's Mainnet, which one gets through the Poly bridge to upgrade to POLYX on the Polymesh Mainnet. This network and its token are intended to be the real deal where individuals and other entities represent real world value and where actions have consequences.
