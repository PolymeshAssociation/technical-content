...
A **Polymesh "DID"** (user ID) can be an individual or an organisation. Organisations always act through the people and automations that are authorised to act on their behalf.

This brings us to the first important pillar that will support our understanding of Polymesh Identities:

> A **DID** represents an individual or an organisation. There may be one or more actors with unique signatures and defined permissions who act on behalf of the DID.
...
The corresponding on-chain concept in Polymesh is the **Polymesh "distributed ID" (DID)**, implemented as a large number and representing an account. You may not choose "petrolhead1980", but it is sufficiently obfuscated that you can keep a good level of privacy.
