...
A **Polymesh "uID"** (user ID) can be an individual or an organisation. Organisations always act through the people and automations that are authorised to act on their behalf.
...
You, as a person, already use several online services, it could be emails, social networks, banks, etc.. For each of those services, you happen to have chosen a username. It might be your first and last names. It might as well be "petrolhead1980". You might in fact have two distinct usernames on the same platform, email say, which you use for different purposes. You are in control of these usernames and they represent you digitally. But they are not you the person.
...
Certain services, banks say, do not entertain the idea of having each customer create more than one username. In similar fashion, certain securities require reporting of holdings per person and entity. Per uID if you will. While Polymesh allows you to have multiple accounts on the network, there is also a cryptographic mechanism that lets you self-report any relevant link between your Polymesh accounts to selected third parties.

<HighlightBox type="tip">

Take a look at **PUIS** lower down for a description of the implementation.
...
Only a Pedersen (cryptographic) commitment of the uID is published on-chain as part of the CDD claim. Polymesh users do know their uID and can use this knowledge to reveal to asset issuers if they are investing in a single asset under multiple Polymesh accounts. Asset issuers can enforce that a user does this for all identities investing in their asset.
...
To take a closer look at how identity and confidentiality play together in Polymesh, this documentation on [confidential identity](https://developers.polymesh.live/confidential_identity/) includes some more information on PUIS, CDD claims, and the linking of identities.
