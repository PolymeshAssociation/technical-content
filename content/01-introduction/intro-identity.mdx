...
Certain services, banks say, do not entertain the idea of having each customer create more than one username. In similar fashion, certain securities require reporting of holdings per person and entity. Per uID if you will. So, while Polymesh allows you to have multiple accounts on the network, there is also a cryptographic mechanism that lets you self-report any relevant link between your Polymesh accounts to select third parties. See PUIS lower down for a description of the implementation.
