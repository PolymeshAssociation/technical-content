...
An example of a Sybil attack in the realm of regulated securities could be accumulating a sizeable stake in the security while avoiding reporting requirements. The Polymesh Unique Identity System (PUIS) is designed to repel this type of attack.

The PUIS is an off-chain process that records hashes of PID attestations. Attestation requirements are defined by the securities themselves, or more precisely the originators of those securities. Since the attestations originate from KYC providers, they are expected to be trustworthy and accurate. The attestations are hashed to minimise metadata leakage and then compared to a database of observed hashes.
...
Polymesh is designed to welcome existing KYC service providers into the Polymesh network. The API integrates easily with existing systems, which allows KYC service providers to **issue on-chain attestations**.
