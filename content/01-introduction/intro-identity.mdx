...
Just as proverbial fish seldom thinks about water, we seldom find ourselves pondering the meaning of such a statement. But, when we are talking about systems that "prove" certain things, it's important to understand exactly what is being proven, and how it is being proven.
...
Will any document suffice? It would be more accurate to say that only documents that emerge **from known issuers with strict processes** can provide strong evidence. For this reason, government-issued photo IDs are a common mean of identification.

If one is presenting a passport as evidence of citizenship, we can say with certainty that the passport office of a certain sovereign state believed, at the time of issuance, that the person's citizenship was as specified in the document.
...
--> Include Diagram …
what it says, plus what we know about the origination process, plus the reputation of the issuer = Strength of Evidence
...
In Polymesh, **descriptive properties of a PID** are called **attestations**. An attestation is a statement about the PID and it is always issued by another identity. As we will see, Know Your Customer (KYC) and Customer Due Diligence (CDD) providers execute their own internal processes to confirm the facts and then issue attestations. This enables a PID to prove something about themselves in (approximately) the following cryptographically provable format:
...
Let's consider ACME Corp, an entity with a PID. We can know the true legal name of the corporation behind a PID because a KYC service provider attests that they have reviewed the pertinent documentation, for example, the Certificate of Incorporation.

The organisation can own things and do things. Let's say that Alice works for ACME Corp. Alice is an individual who can own things and do things for herself (her PID) as well as for ACME Corp, being an employee of the organisation. When she is acting on behalf of ACME Corp, within the limits of her authority, she signs with a signing key by ACME Corp, on behalf of ACME Corp. When she is acting on her own behalf, she uses a self-assigned signing key associated with her personal PID.
...
* Or a _multi-signature_ key in that it is based on _m_ keys, of which _n_, the quorum, must agree for any action to be validated.

--> Illustration to be included

<HighlightBox type="info">

For a closer look at identity on Polymesh, authentication and keys, as well as primary and secondary keys, and different key types, take a look at [https://developers.polymesh.live/identity/](https://developers.polymesh.live/identity/).

</HighlightBox>

## Portfolios

Polymesh Identities optionally organise owned assets into **portfolios**. Portfolios are **logical containers** that provide a way to define permission boundaries as well as organise reporting.
...
An example of a Sybil attack in the realm of regulated securities could be a single entity accumulating a sizeable stake in the security with the use of multiple PIDs, while avoiding reporting requirements. The Polymesh Unique Identity System (PUIS) is designed to repel this type of attack.

The PUIS is an off-chain process based around an off-chain database. This database can only be accessed by CDD providers. CDD providers can insert new customers, or query for existing ones by using so-called identity _fingerprints_ unique to their customer. In this PUIS, the CDD customer's unique id, or uID, is a v4 UUID generated once when the customer is on-boarded for the first time. It is by this uID that customers are indexed in the PUIS system, and uIDs too always remain off-chain.

Only a Perdersen (cryptographic) commitment of the uID is published on-chain as part of the CDD claim. CDD customers do know their uID and can eventually reveal it, off-chain, to asset originators that have compliance rules about the investing entities; as opposed to compliance rules about the investing PIDs. In this way, the PUIS lets investors use multiple PIDs, for privacy reasons say, while enabling certain compliance rules about entities.
...
Customer Due Diligence, or CDD, is a minimal KYC attestation that entitles a PID to interact with the Polymesh network. Functions with material importance are restricted to PIDs that have completed CDD with an approved CDD service provider.
