...
slug: /introduction/intro-governance
---

_In blockchains, the word governance describes the rules, process management and decision-making responsibilities by which the developers and maintainers keep the chain evolving._

It is always a difficult but equally important topic. This is especially true for blockchains on which participants issue security tokens, as the chain forms the source of truth, i.e. it establishes the state of facts. Additionally, one can expect the environment to evolve, which will require the blockchain itself to evolve and change too.
...
Off-chain is the manner by which early blockchain developers handled governance, and it presents challenges. For instance, in case of disagreements on the nature of changes to the blockchain or their implementation, the rift can lead to separate factions backing more than one implementation. This situation can lead to hard forks, i.e. a change in the protocol of a blockchain that is backward-incompatible, creating more than one source of truth. Were this to happen, there is no current legal or regulatory clarity about deciding the relevant state for security tokens. We can assume, capital market participants and regulators are unlikely to accept this uncertainty.

Next, even after stakeholders have reached consensus on a given change, node operators face a coordination problem when it comes to updating them. Nodes need to deploy their blockchain software within an agreed time-window. Again, node operators need to coordinate off-chain, for this to happen. If they disagree about deployments or the implementation, forks are likely once more.
...
This is a permissioned blockchain, and its operators should be able to come and go. To be able to operate, an operator needs to be validated by the system beforehand. This validation is covered by the governance system, fostering decentralisation.

Chain **upgrades**, i.e. changes to the protocol are decided and implemented by certain stakeholders as delineated in the current governance system. Moreover, the node binary is stored directly on-chain. Meaning an upgrade leads to a change of the on-chain representation of the binary. Thus, each node checks if there is a new binary, and if there is, starts working with that new version automatically. The possibility of non-malicious hard forks is therefore highly unlikely.

Another aspect of Polymesh that adds to the improbability of hard forks is the choice to use WebAssembly (WASM) for the protocol binary. WASM bytecode was initially conceived to enable high-performance applications on webpages. For this reason, large software companies maintain WASM compilers and virtual machines for various operating system platforms. This landscape makes the risk of hard forks caused by compiler or platform bugs highly unlikely.
...
![Governance Process](../features/images/governance-process.png)
