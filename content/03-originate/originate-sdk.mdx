...
* Alice, ACME's CEO and acting agent, already has a Polymesh identity, tied to a primary private key named `ceo`.
* `ceoKey`'s private key is based on the `"word1 word2 ..."` mnemonic.
* A Polymesh client has been [instantiated](https://developers.polymath.network/polymesh-sdk-api-reference/classes/polymesh#static-connect) by Alice the CEO so she can do the next actions:
    <MultiCodeBlock>
...
So when we wrote `await` in `await apiCeo.getCurrentIdentity()`, this is no accident. We indeed need to do a round trip to the blockchain to know what identity our public key is associated with.
