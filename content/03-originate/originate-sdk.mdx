...
However, as the CEO, Alice still needs to do one more step, define the conditions of ownership. Namely, require anyone who acquires the token to not be a resident of Liechtenstein. An exception will be made of the primary issuance agent who is simply used as a conduit.
...
const aliceAsCeoId = await apiAliceAsCeo.getCurrentIdentity();
...
const setRequirementsQueue = await acmeRequirements.set([[
    {
        "condition_type": ConditionType.asIsPresent("isJurisdiction"),
        "issuers": [{
            "issuer": aliceAsCeo.did,
            "trusted_for": TrustedFor.asSpecific("isJurisdiction")
        }]
    },
    {
        "condition_type": ConditionType.asIsAbsent(Claim.asJurisdiction(["Li", "isIdentity"])),
        "issuers": [{
            "issuer": aliceAsCeo.did,
            "trusted_for": TrustedFor.asSpecific("isJurisdiction")
        }]
