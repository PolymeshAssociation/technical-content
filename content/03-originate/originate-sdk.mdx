...
```typescript
const token: SecurityToken = await apiAlice.getSecurityToken({ "ticker": "ACME" });
```

```javascript
const token = await apiAlice.getSecurityToken({ "ticker": "ACME" });
```
...
With this, the token is _originated_. Alice, under her personal account, is not yet a holder of any amount of the security token, though, we remedy that in the next chapter "Distribution".
