...
Given the breadth of operations that a token sees over its lifecycle, it is only natural to spread responsibilities among varied parties, a.k.a. external agents. Each external agent is entrusted with, and restricted to, certain operations. Given a well-defined mix of responsibilities of a given external agent, it is possible to map it to existing real world roles, such as primary issuance agent (PIA) or corporate actions agent (CAA). Polymesh lets you tailor agents' responsibilities and permissions with the conventional use of external agent permission groups.

### The token owners

There is a special group of external agents, one that gives all rights over a token. It helps discussion to call this type of agent the token owner. To avoid ending up with _orphan_ tokens, there must always be at least one token owner. The originator of a token is, quite naturally, the token's initial owner. Eventually, because of the importance of this ownership role, it is expected that:

* The token owner will be held by a multi-signature account.
* The token owner would be called upon for rare and higher-level tasks, typically to appoint and revoke external agents.
...
As seen above, the token represents the type of the asset, and is _owned_ by its originator, or by whomever else they granted full rights on the token to. By contrast, the asset it represents is owned by individual investors' accounts.
...
When the total supply of tokens increases, it is called **issuance**. The external agent account responsible for it is commonly called the **primary issuance agent (PIA)**. The owner of the token itself can act as a PIA, although this role can be delegated by the owner.
...
Whether an asset can be transferred, or owned at all, is called **compliance** and is automatically enforced on-chain by Polymesh. In effect, for an action to take place, all involved accounts must have attestations that match with the rules defined by the token originator for that action. Skip ahead to the section about [Compliance](compliance) if you want details about this.
