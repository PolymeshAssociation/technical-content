...
All assets are differentiated by a **ticker registration**. The ticker is a 12 character identifier. Tickers can be registered by any user and the registration remains valid until their expiry date, currently six months after the initial registration. This usually occurs when asset origination is finalised. Once a ticker is registered and as long as the registration remains valid, no other user can issue an asset with that specific ticker. Additionally, transferring ticker registration to other identities is possible.
...
* the individual investor balances of each account invested in an asset.

## Asset origination

The **issuer** of an asset is the account that registered the ticker for the asset and created it. Ownership of an asset can be transfered to another account through a ```TransferAsset``` authorisation. Once the target account accepts the transfer, ownership changes.
...
* conduct a forced transfers of ownership from any investor back to the primary issuance agent of the asset.

<HighlightBox type="info">

The issuance agent is an account responsible for treasury management and token distribution. The agent is elected by the asset issuer.
...
To originate an asset, not only is a ticker necessary but, furthermore, the type of asset (equity, bond, fund) has to be specified. Every asset is initially created with zero total supply.
...
Compliance is established at the account-level. Thus, receiving or paying with tokens requires that all involved accounts must have claims matching the compliance rules for the asset. Those claims must be attested to by the asset's appointed KYC service provider. The compliance rules are specified by the asset issuer. Asset compliance is, therefore, flexible within a standardised structure and enforced on-chain.

Accounts play a large role in asset origination, as every asset is **associated with two accounts and one unique ticker**. The **asset issuer's and primary issuance agent's accounts** are both linked to the asset. The account that created the asset and the one responsible for distribution are linked to the asset. Both roles can be fulfilled by the same account, and the asset issuer can change the primary issuance agent.

It is also possible for the asset issuer to opt for a primary issuance agent for the asset with a different account. The new account holder has to accept this role before the change can be effected.
...
In Polymesh, an asset checkpoint is a compilation of the balances of an asset at a given point of time (in effect at a certain block height). The record of balances equal to the total balance of the asset and the balances held by the different identities.

### Why do we need asset checkpoints?

Asset checkpoints help us establish a record of the asset ownership at a point in time. This can then be used for capital distribution and corporate ballots.

### How is a checkpoint created?

If a checkpoint was actually making a copy of the blockchain data it would consume a lot of computing resources. Instead, a checkpoint is a marker, which is extremely quick to create and finalise in a block. This checkpoint-as-a-marker only records the total supply of the asset. Subsequently, after the checkpoint, if a transaction on the asset modifies an identity's held balance, a single record is added to the checkpoint with the values _before_ the change, i.e. the values that existed at the time of creation of the checkpoint. In computing terms, the process is called **lazy checkpointing**.
...
Scheduled checkpoints are defined with a starting timestamp (in seconds) and a regular interval period. The period can be a multiple of seconds, minutes, hours, days, weeks, months, or years. If the period is of zero length, then the schedule is understood as non-recurring and a single checkpoint is created at the scheduled start.
