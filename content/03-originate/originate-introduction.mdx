...
What happens if no transactions are made? Then no records result.

With the latrer, a checkpoint is scheduled. The schedule is defined with a starting timestamp (in seconds) and a regular interval period for recurring checkpoints. The period can be a multiple of seconds, minutes, hours, days, weeks, months, or years.
