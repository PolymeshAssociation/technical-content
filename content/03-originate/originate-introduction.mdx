...
When the total supply of tokens increases, it is called **issuance**. The external agent account responsible for issuance is commonly called the **primary issuance agent (PIA)**. The owner of the token itself can act as a PIA, but this role can be delegated by the owner.
...
Other actions address rare cases such as the forcible return of the asset to the primary issuance agent. For the avoidance of doubt, issuance bypasses the compliance step.
