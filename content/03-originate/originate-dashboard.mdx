...
You can now proceed to specifying the details of the security this token represents. In practice, not all important business decisions will be finalized at this point. That's okay. You can return any time in the next 60 days while the reservation remains in effect. 

![returning later](images/16-returning-later.PNG)

Let us proceed with a simple configuration. 

Take a moment to explore the options. This will help you understand the breadth of security tokens that can be created this way. To matters simple, enter just a company name, select "Equity Preferred", and leave the rest of the defaults as they are. 
