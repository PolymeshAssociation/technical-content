...
This is where ACME defines the rules of ownership. Polymesh will ensure that no transfers complete unless these conditions are met. A holder can transfer assets to another account that is non-compliant but the transaction will remain in a pending state until the compliance requirements are met or the sender cancels the transfer.
...
Let us suppose that ACME wishes to exempt primary issuances and create two compliance rules. The rules will be that all token owners must have attestations from the KYC service provider and that they must not be in Liechtenstein.
