...
Returning to Alice's wallet, simply click <kbd>Add a key</kbd> and follow the usual procedure. Assign an internal name that will help Alice remember what the key is for, e.g. ACME CEO. The result will be that Alice has a second, unassigned key. **Do not** assign this key. There is nothing Alice can do to associate the key she just made with ACME, a PID she does not control. Instead, let us say that Alice provides they public key to ACME through an off-chain communication channel and ACME adds it to _their identity_ on their own authority.
