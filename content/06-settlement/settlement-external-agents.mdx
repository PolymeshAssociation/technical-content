...
Here we can see that ACME has one agent, the agent with full permission. The agent is the ACME Corp account. The ticker input field for the call is the symbol "ACME" in hex (0x41434d450000000000000000).
...
3. Input the ticker symbol. At the time of writing, this is required to be in hex format, which you can get from the query above. It should be 12 bytes long with trailing zeroes.
...
5. Input the ticker symbol as previously used (in hex format, 12 bytes long with trailing zeroes if required).
6. Finhally select `Custom` for the agent group and enter the number for the group ID previously created.
