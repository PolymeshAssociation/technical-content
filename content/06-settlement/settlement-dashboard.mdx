...
Polymesh is designed to intregrate with existing systems that the counterparties operate in-house. Let us consider a deceptively simple-sounding example of an online exchange to highlight the many organisations and processes that _could be_ involved.
...
### Create Bob
 ...
 Alice will transfer some of her shares to another user. For this to work, we will need another user, so create a user with a new identity, Bob, complete [customer due diligence](/quickstart/quickstart-cdd) and [get some POLYX](/quickstart/quickstart-polyx)  because Bob will need to sign some transactions.
...
Transfers in Polymesh do not occur in a void. Each transfer occurs in a venue, which is an organised trading facility (OTF), regulated market (RMs), or multilateral trading facility (MTF) that receives instructions from counter-parties for the purpose of matching, to convert separate orders into executed trades. 

In summary, venues are logical groups of instructions and settlements. The creator of a venue controls permission, meaning who is allowed to create settlements within the venue. When ACME transferred shares to Alice, the token studio created a venue for ACME. By default, only ACME has permission to trade there. We can safely assume that Alice doesn't have permission at this time. 
...
At this moment, it suffices for us to spot our recent transaction in the list. There are many ways to query the blockchain to find such a detail in the future. See if you can discover on your own how to use `Developer`/`Chain state` to query the venues where Alice is _allowed_ to create a settlement. Hint: The query is about settlements and user venues.
...
Let us transfer 1,000 shares to Bob.
...
If this instruction involved 2 or more asset transfers, then we would create additional legs. Instructions settle atomically, meaning all legs execute completely. There is **no case in which an instruction is partially processed** or some, but not all legs are processed.

</HighlightBox>

Alice will transfer 1.000 shares to Bob. Notice that the number entered can be scaled. To the right, `kilo` would multiply the input value by 1,000. Leave it as POLYX, which we will take to mean `1:1` in this context.
...
The transaction that creates the instruction is created and confirmed, but the trade has not occured yet. This is an important difference between Polymesh and many other distributed blockchain-based systems. In other cases, there is nothing Bob can do to prevent the arrival of assets in his portfolio, which is problematic in the context of regulated securities. In Polymesh, Bob needs to affirm the instruction and Bob can reject the instruction if he disagrees with it. 

Let us have a look from Bob's perspective now that this instruction exists. 

Change to Bob's Polymesh DID in the wallet and navigate to the `Portfolios` tab in the dashboard: https://dashboard.polymesh.live.
...
Since this the first time Bob has received ACME shares, Bob will create a uID scope for ACME. The system leads Bob through the process. Also, Bob may wish to move some or all of his shares to another portfolio. For details on these processes, return to [Token Studio Dashboard](/distribute/dashboard) in the Distribution section to recall how Alice received her first ACME shares and moved them to her cold store portfolio. 
