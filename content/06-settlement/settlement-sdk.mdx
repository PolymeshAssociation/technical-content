...
This test will unfortunately also test whether Alice can receive new ACME shares, which is one irrelevant hurdle SafeHands does not care about.
...
On the other hand, if they want to preemptively change the order on NextDaq as soon as a conflicting pending instruction pops up, they have to go differently. There is an added difficulty around the fact that `.getLegs` is an `async` function, but `.filter` does not take `async` predicates. So we use an array of promises that yields an array of array, which can be flattened.
