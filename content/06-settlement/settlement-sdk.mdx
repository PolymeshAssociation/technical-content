...
Here, Alice, the individual, wants to start trading and exchange some of her ACME shares in exchange of US dollars. For that, she enlists the services of a broker named SafeHands who has access to an exchange named NextDaq. She plans on instructing SafeHands to list 1,000 of her shares for at least 5 USD apiece, and let them handle the reception of USD and the transfer execution in a timely manner.
...
const aliceInstruction: Instruction = pendingInstructions.find((instruction: Instruction) => {
    return instruction.id.isEqualTo(tradeInstructionId);
});
```
...
const payeeFolio: Portfolio = new Portfolio({
    "did": paymentLeg.from.did,
    "id": paymentLeg.from.id
});
const payeeCustodian: Identity = await payeeFolio.getCustodian();
// Is someone trying to pay us with our own money?
// Or with money from one of our clients?
if (paymentLeg.from.did === safeHandsDid || payeeCustodian.did === safeHandsDid) {
...
const payeeFolio = new Portfolio({
    "did": paymentLeg.from.did,
    "id": paymentLeg.from.id
});
const payeeCustodian = await payeeFolio.getCustodian();
// Is someone trying to pay us with our own money?
// Or with money from one of our clients?
if (paymentLeg.from.did === safeHandsDid || payeeCustodian.did === safeHandsDid) {
