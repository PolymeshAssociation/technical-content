...
For the avoidance of doubt, yes, an instruction can have been affirmed by all parties and still stay in a pending state while the KYC requirements are unresolved. The reason behind this durability of the instruction is that it represents a contractual agreement between parties. As opposed to the compliance of parties, which is fluid, what with expiration dates, and failing to comply is but a temporary setback that has to be fixed by contractual obligation.

As a matter of detail, when an instruction has been fully affirmed but is still pending because of some missing compliance, and it is finally possible for it to execute because the last compliance requirement has now been satisfied, the instruction does not automagically execute. It still needs an account to explicitly send a transaction to execute it. Naturally, if an instruction is asked to be executed and it still fails because of compliance, then the instruction is not cancelled, but is instead sent back to a pending state.
