...
- Create DEEPUSD. Choose a unique symbol to use whenever DEEPUSD is mentioned. 
...
Using an account with authority, for instance a CFO secondary key. In `Extrinsics`, select the `corporateAction` group, and `initiateCorporateAction(...)` on the right. Then complete the fields as follows:
...
* decl_date: Put the same time as the checkpoint Moment. This is Unix time which are seconds past a certain time in the past. Tools like this https://www.epochconverter.com/ can help convert. You need to specify the time in milliseconds, so apend 000 because each second is 1,000 milliseconds.
