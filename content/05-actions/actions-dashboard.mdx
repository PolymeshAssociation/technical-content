...
Astute readers will note that is an asset transfer. Therefore, it does not occur without Alice's consent. Also, any compliance rules attached to DEEPUSD apply. We did not define any compliance rules for DEEPUSD and Alice's consent is implied since she is explicitly requesting the dividend. Everyone who holds a given asset needs a unique ID scope, so Alice will create a uID scope for DEEPUSD. Return to the asset [distribution section](/distribute/dashboard) for a more complete description of this process. 
