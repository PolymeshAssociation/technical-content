...
The ACME token has been trading for a while, and the company has had a very good quarter. The board has decided to distribute a dividend of US \$5,000. Each shareholder will be entitled to receive a portion of the dividend in proportion to the number of shares they own. Since ACME distributed 20,000 tokens, this is \$0.25 per token. ACME will use Polymesh to execute the distribution.
...
The dividend distribution requires Alice's consent. When she accepts the dividend, she will first create a uID scope for ALCY (if this is the first time she has received it), and then accept distribution. The funds arrive in her default portfolio. 
