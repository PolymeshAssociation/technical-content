...
Polymesh provides specialized extrinsic pallets that deal directly with the two most common types of corporate actions:

- **Capital distributions**: Capital, usually in the form of another asset, is distributed to asset holders in proportion to their holdings. 
- **Corporate ballots**: Asset holders are invited to vote on motions. 

In addition to these specialized pallets, more generalized pallets support increased implementation flexibility. 
...
- **Predictable benefits**: Fixed rate benefits where the amount is established, and does not change over time. 
- **Issuer notice**: Announcements that the issuer wishes to record on chain for reasons of permanence. 
- **Reorganization**: Reorganizing the shares such as in mergers and acquisitions and stock splits.

Finally, a generic corporate actions pallet supports the creation of novel corporate actions:

- **Other**: A generalized type that supports actions that are not addressed directly.
...
The corporate actions pallet is the base layer, while capital distribution and ballets are specialized layers that leverage the base later. 
...
All begins with ```initiate_corporate_action```. This dispatchable function creates a corporate action. Tthe following is included:

* an associated asset's ticker,
* free-form text, and
* a corporate action type specification.

Additional information can be provided in regard to the record date, targets of the corporate actions, and tax witholdings.
...
For efficiency, checkpoints use a "lazy update" algorythim. Consider Alice's current balance and a scheduled checkpoint executing now. It follows that Alice's that Alice's balance at the checkpoint time is equal to her current balance. The algorythim can report her balance at the checkpoint with no further work. It is necessary to record Alice's previous balance at the checkpoint time when something occurs that changes Alice balance. When this happens, Alice's previous balance is written to the checkpoint in the past before her current balance is adjusted. In this way, balances at previous checkpoints are always recoverable. 

</HighlightBox>

Now, let's take a look at regulatory specification possibilities with Polymesh: witholding taxes, defining the targeted token holders, and linking associated documentation.

### Taxes and corporate actions

Tax regulation varies between jursidictions. Sometimes the issuer of an asset or the CAA have the regulatory responsability to deduct taxes from dividends and transfer them to the relevant government agency. Polymesh supports witholding of taxes.

Witholding taxes is performed natively on Polymesh. One can specify a percentage of an asset distribution to withhold.

Such rules can apply to all investors as a generall specification or to groups of individual investors. 

- ```set_default_witholding_tax``` creates a general tax witholding rule.
- ```set_did_witholding_tax``` creates rules that apply to specific investors.

### Specifying the token holders targeted

Corporate action participants can be specified. This can be helpful when a corporate action targets only a specifc group of investors.
...
Free-form text provides a succinct human-readable description of the corporate action. This is useful in contexts such as browsing the corporate actions to find a specific action of interest. The text is not expected to provide a comprehensive description in all cases. Relevant documentation such documentation to satisfy reporting requirements can be linked to a corporate action with```link_ca_doc```.
...
The preference of the investor, Alice, is to accept the motion with an amendment, then to not accept the motion withoud amendments. Alice doesn't want to accept the motion in its current form.
...
Motions are independent of each other. The whole voting power of the tokens an be used to weigh each motion seperately.
...
Alice's 100 tokens could represent 100% ownership or an extremely small holding. The voting power Alice receives from her 100 tokens considers her tokens as portion of the total voting power of all tokens which is always 100%. In this context, "weight of" is way of summarizing that Alice wil receive her fair share of voting power. 
...
Option A is the most preferred. If Option A doesn't have enough votes to be adopted and is discarded, then the 75 votes of Option A would fall to Option C. This changes Alice's vote to: Option B has 5 votes, Option C has 95 votes. RCV ballots are an efficient method of implementing a process that emulates multiple rounds of voting since the voters are able to define contingencies with a single vote. 
