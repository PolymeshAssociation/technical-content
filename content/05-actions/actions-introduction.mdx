...
For example, an investor, i.e. token holder, of ACME tokens is asked to vote on a motion to distribute benefits. The investor holds 100 tokens.

The preference of the investor is to accept the motion with an amend, then to not accept the motion withoud amends. The investor doesn't want to accept the motion in its current form.

The investor weighs its vote in the following manner:
...
There are two motions: one to distribute capital now and one to elect a new CAA. The investor weighs motion 1 in the following way:
...
The investor was able to use all 100 tokens for weighing for each motion seperately.

Is there another option to configure a ballot?

A ballot can be configured with ranked-choice voting (RCV). This allows voters to specify that the weight, assigned to a choice not making it, should fall back to a different choice. For example, consider 3 choices paired with weights: - a) 49 votes, no fallback - b) 100 votes, fallback to a) - c) 120 votes, no fallback.
