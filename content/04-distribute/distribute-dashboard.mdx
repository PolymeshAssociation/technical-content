...
In _[Secondary keys](/originate/secondary)_, we created distinct accounts for Alice and ACME, and Alice created a secondary key to use when she signs on behalf of ACME as the CEO.
...
**Multiple distributions can be logically grouped into a single event**, for example "Founder distributions". Since Alice and only Alice will receive shares, let us call this event "Alice's founder's shares".
