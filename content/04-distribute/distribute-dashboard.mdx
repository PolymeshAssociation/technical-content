...
In the [secondary keys section](/originate/secondary), we created distinct identities for Alice and ACME, and Alice created a secondary to use when she signs on behalf of ACME as the CEO. 
...
Now it time for the distribution agent, which is ACME, to distribute the tokens. Click `Distribute`. 
...
Switch to Alice's personal Polymesh ID by bring her personal primary key into focus in the wallet. 
...
1. Tokens are minted by originators and are distributed by distribution agents which can be the same organization. 
