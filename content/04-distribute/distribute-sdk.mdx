...
Now, Alice doesn't want to receive her ACME shares into her default portfolio. Instead, she wants to have them sent to a new "cold store" portfolio, she first needs to [create](https://developers.polymath.network/polymesh-sdk-api-reference/classes/portfolios#create) the portfolio.
...
```typescript
const checkpointQueue: TransactionQueue<Checkpoint> = await token.checkpoints.create();
const checkpoint: Checkpoint = await checkpointQueue.run();
const postDistributionCheckpointId: string = checkpoint.id.toString(10);
```

```javascript
const checkpointQueue = await token.checkpoints.create();
...
```typescript
const token: SecurityToken = await apiBob.getSecurityToken({
    "ticker" :"ACME"
});
const allCheckpoints: CheckpointWithCreationDate[] = await token.checkpoints.get();
const postDistributionCheckpoint: Checkpoint = allCheckpoints
    .map((checkpointInfo: CheckpointWithCreationDate) => checkpointInfo.checkpoint)
    .find((checkpoint: Checkpoint) => checkpoint.id.toString(10) === postDistributionCheckpointId);
...
```javascript
const token = await apiBob.getSecurityToken({
    "ticker" :"ACME"
});
const allCheckpoints = await token.checkpoints.get();
const postDistributionCheckpoint = allCheckpoints
    .map((checkpointInfo) => checkpointInfo.checkpoint)
    .find((checkpoint) => checkpoint.id.toString(10) === postDistributionCheckpointId);
