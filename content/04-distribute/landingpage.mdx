---
title: Distributing Assets
description: Distribute regulated security tokens
slug: /distribute/landingpage
---

import OverviewBox from "../../src/components/OverviewBox"
import ActionCard from "../../src/components/ActionCard"
import ActionCardWrapper from "../../src/components/ActionCardWrapper"

Discover how to distribute regulated security tokens with Polymesh.

What’s inside?
* An overview explaining the **main concepts**.
* Distribute an asset with an introductory exercise using the Dashboard.
* Learn how to integrating distribution processes into existing systems using the SDK.
* A quiz to self-assess acquired knowledge.
* Helpful links to reference documentation.


***

<OverviewBox title="Overview: Distributing assets with Polymesh" linkHref="/distribute/introduction" linkText="Overview" imgSrc="/illu-p-screen.svg">

Now that you know how to originate an asset, distribute it! This overview introduction explains how Polymesh addresses security token distribution.
</OverviewBox>

<h3>Asset Distribution</h3>
Now that you have originated an asset, it’s time to distribute!

The example scenario takes you through the possibilities Polymesh offers. Execute your first distribution using the Dashboard, and then discover the integration possibilities.

<ActionCardWrapper>
	<ActionCard href="/distribute/dashboard" text="Distribute an asset using the Dashboard" desc="Distribute assets, such as dividend payments with your browser and the Polymesh Dashboard."></ActionCard>
	<ActionCard href="/distribute/sdk" showDevSign="true" text="Distribute an asset with the SDK" desc="Integrate asset distribution into your systems with the SDK."></ActionCard>
</ActionCardWrapper>

<OverviewBox title="Dive into further resources" linkHref="https://developers.polymath.network/polymesh-sdk-api-reference/" linkText="Explore" imgSrc="/illu-p-code.svg" external="true">

A closer look at the detailed technical documentation to dive deeper?
</OverviewBox>
