...
Suppose ACME chose EzKYZ to perform this service. The agreement would likely specify how EzKYC will perform their duties. The _result_ of that process would be that EzKYC will _sign_ an _attestation_ along these lines (not as implemented):
...
CDD providers are admitted to the network via the [governance](/introduction/intro-governance) process. In summary, any organisation can become a CDD provider by achieving the support needed to carry a motion. In practice, this is expected to mean that a concensus forms around allowing the organisation to perform the service in the manner in which they propose to perform it. Such consensus would reflect a belief that it will strengthen the network overall.
